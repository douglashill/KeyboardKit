...
class ScrollViewKeyHandler: InjectableResponder, UIScrollViewDelegate {
    ...
    private lazy var nonDiscoverableZoomingCommands: [UIKeyCommand] = [
        ...
        UIKeyCommand(KeyboardInput.zoomIn.keyEquivalent, action: #selector(kbd_zoomIn)),
        // This is the one users are expected to press. This is a hyphen.
        UIKeyCommand(KeyboardInput.zoomOut.keyEquivalent, action: #selector(kbd_zoomOut)),
    ...
    static let actualSizeKeyCommand = DiscoverableKeyCommand(KeyboardInput.zoomToActualSize.keyEquivalent, action: #selector(kbd_resetZoom), title: localisedString(.scrollView_zoomReset))

    static let refreshKeyCommand = DiscoverableKeyCommand(KeyboardInput.refresh.keyEquivalent, action: #selector(kbd_refresh), title: localisedString(.refresh))
